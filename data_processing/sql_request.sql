-- Скрипт запроса на получение всех продуктовых предложений,
-- задач и сделок в рамках определённых продуктов и за
-- определённую дату (+ доп. обработка).
-- Сам запрос используется в pyspark_request.ipynb
-- в качестве аргумента для sql-запроса в spark.
with
...
select cast(productoffer.ucpid as string) as epk_id,
       ...
       replace(replace(productoffer.productofferdescription, e'\r', ''), e'\n', ''),
       productoffer.productofferstatuscode,
       productoffer.productofferid,
       productoffer.productgroupcode,
       replace(replace(productoffer.closingcomment, e'\r', ''), e'\n', ''),
